...
$subquery = "select sr.fileloc,sr.title, mgc.id from genres, map_genre_content as mgc, show_reference as sr where genres.id = mgc.genre_id and mgc.myubi_id = sr.myubi_id";
...
while($entry = mysql_fetch_assoc($subres)){
	...
	if(mysql_num_rows($res) > 0){
		...
		$update = "UPDATE map_genre_content SET myubi_id='".$showId['myubi_id']."' WHERE id=".$entry['id'];
