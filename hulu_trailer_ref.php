...
$userRating = $showinfo->video->rating;
		...
		if($fee=="no" && $type=="film_trailer" && $network != "crackle-movies" && $userRating > 2){
			$title  = $showinfo->video->title;
			$title  = str_replace('Movie Trailers: ', "", $title);
			$season = "x";
			$epnum  = "x";
			...
			if(stripos($title,"'")){
				...
				$query = "select TITLE from trailer_content where TITLE LIKE '".$reg."' AND NETWORK='".$network."' AND PROVIDER='".$provider."'";
				...
				$query = "select * from trailer_content where TITLE='".htmlentities($title)."' AND NETWORK='".$network."' AND PROVIDER='".$provider."'";
				$res = mysql_query($query);
			}
					...
					if(mysql_num_rows($res) > 0){
						print "<b>title has already been stored</b> <br>";
							...
							$surl      = getURL($showinfo->video->title,$urlid);
							$genre     = getGenre($surl);	
							$desc      = getDescription($surl,$showinfo->video->title);
...
function saveShow($_myubi,$_title,$_type,$_keywords,$_desc,$_network,$_showthumb,$_thumbnail,$_keyart,$_provider,$_genre,$_url,$_pubDate,$_year,$_urating,$_rating,$_expire,$_runtime,$_urlid,$_embed){
 ...
 $query = "insert into trailer_content (MYUBI_ID,TITLE,QUALITY,DESCRIPTION,NETWORK,RATING,YEAR,URL,THUMB,SHOWTHUMB, RUNNINGTIME,PROVIDER,GENRE,URLID,PUBDATE,FEE,EXPIRE,ASPECT,EMBED) VALUES ('$_myubi','$_title','HD','$_desc','$_network','".$_rating."','$_year','".$_url."','".$_thumbnail."','".$_showthumb."','$_runtime','$_provider','$_genre','$_urlid','$_pubDate','0','".$_expire."','9','".$_embed."')";
			 
				if (mysql_query($query)) {
					
					$query = "select * from trailer_content where TITLE='".$_title."' AND NETWORK='".$_network."'";
					$res = mysql_query($query);
