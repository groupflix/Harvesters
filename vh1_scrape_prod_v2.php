...
$logger->info("vh1_scrape_prod.php: Starting..." );
...
$rows = $scrape->fetchAllBetween($start,$end,$data,true);
	...
	foreach($rows as $showpage){
		...
		$regexp = "<a\s[^>]*href=(\"??)([^\" >]*?)\\1[^>]*>(.*)<\/a>"; 
		preg_match_all("/$regexp/siU", $showpage, $matches);

			$obj->title			 = findTitle($matches[3][0]);
			$obj->link 			 = $vh1.$matches[2][0];
			$obj->img_string	 = findImageString($obj->link);
		...
		if($obj->img_string	!= "default"){
			print ".........checking " . $obj->title	 ." show data...........";
			GetShowData($obj,"episode");
		}
...
$query = "update harvester_index set last_update='".date('Y-m-d')."' where provider='VH1'";
...
function GetShowData($showObject,$type){
    ...
    $epObject->caption 	= 0;
    $epObject->cid		= "0";
	$epObject->expire  	= $harvest->addDate(date('Y-m-d'),2);
	$epObject->fee		= "0";
	...
	$logger->info("vh1_scrape_prod.php:getShowData - Link: " . $epObject->url);	
	$tags     = get_meta_tags($epObject->url);
	

	if($tags['mtvn_fullep_lnk']){
			...
			$seasons  = array();
			// Check for Episodes
			if(stripos($fulleps,"video.jhtml?sort=descend")> 0 || stripos($fulleps,"video.jhtml?filter=fulleps")>0){
				
			}else{
				$logger->info("vh1_scrape_prod.php:getShowData - No Shows: " . $epObject->url . " skipped!");
				return "no shows";
			}
			...
			if(stripos($fulleps,"season") > 0){ 
				...
				$logger->debug("vh1_scrape_prod.php:getShowData - Seasonal show. ");
				...
				foreach($_seasons as $num){
					...
					$num    = substr($num,$numpos+7, strlen($num));
					$logger->debug("vh1_scrape_prod.php:getShowData - Season: " .$num . " available.");
				...
				$logger->debug("vh1_scrape_prod.php:getShowData - Not seasonal show.  Grabbing single season");
				$season = "1";
				...
				$num_urls = 0;
				$logger->debug("vh1_scrape_prod.php:getShowData - No Shows: "  . $epObject->url . " skipped!");
				return "no shows";
			...
			if(file_get_contents($huluurl)){
				$isHulu = true;
				$logger->debug("vh1_scrape_prod.php:getShowData - is HULU! ");
			}
			...
			$logger->info("vh1_scrape_prod.php:getShowData - Going to getIMDBid: " . $object->title);
			$imdbID = $harvest->getIMDBid($epObject->title, 1);
			//=============1. GET IMAGES WITH IMDB ===========================
			if ($imdbID  != "error") {
				$logger->info("vh1_scrape_prod.php:getShowData - getIMDBid found! ");
			...
			if($epObject->description == ""){
				if($epObject->synopsis == "" ||strlen($epObject->synopsis) < 150){
					$epObject->epsynopsis  = $epObject->synopsis;
				...
				for($k=0; $k<$num_urls ;$k++){
					...
					foreach($episodes as $entry){
						...
						if ((int)$obj->year < (int) $show_year){
							...
							$logger->debug("vh1_scrape_prod.php:getShowData - Updated show year: " . $seriesObj->year);
					...
					if($ep_ref_exists == false){
						$logger->info("vh1_scrape_prod.php:getShowData - No Reference Present - Inserting : " . $seriesObj->fileloc);
						saveEpisodeRef($seriesObj);
					}else
						echo ".....vh1_scrape_prod.php:getShowData - Show ref is already present....";
...
function GetEpisodeData($entry, $epObject) {
	...
	$logger->debug("vh1_scrape_prod.php:GetEpisodeData - Episode traverse: " . $entry);
	...
	$logger->debug("vh1_scrape_prod.php:GetEpisodeData - Episode title: " . $epObject->episodeTitle);
	...
	preg_match_all("/id=(.*)\"/siU", $entry, $urimatch);
	$uri    = explode("/",$uri);
	$uri    = "mgid:uma:videolist:vh1.com:".$urimatch[1][1];
	$_uri   = explode(":",$uri); 
	...
	$url		= $vh1 . substr($entry, $urlpos, $urlpos2-1);
	$epObject->url = $url;
	...
	$logger->debug("vh1_scrape_prod.php:GetEpisodeData - Description: " . $epObject->description);
	...
	if(is_nan($ep_info[0])){
		...
		if(substr($ep_info[0],0,1) == $epObject->season)
			$epObject->epnum   = (int) substr($ep_info[0],1);
		else
			$epObject->epnum    =trim($ep_info[0]);
	}
	
	print "----------------epnum " . $epObject->epnum . " ------------------";
	$logger->debug("vh1_scrape_prod.php:GetEpisodeData - Episode: " . $epObject->episode);
	$logger->debug("vh1_scrape_prod.php:GetEpisodeData - Epnum: " . $epObject->epnum);
	...
	$logger->debug("vh1_scrape_prod.php:GetEpisodeData - pubdate: " . $pubdate);
	...
	$logger->debug("vh1_scrape_prod.php:GetEpisodeData - pubdate dateFormatted: " . $epObject->pubDate);
	...
	$logger->debug("vh1_scrape_prod.php:GetEpisodeData - keywords: " . $epObject->keywords);
	$logger->debug("vh1_scrape_prod.php:GetEpisodeData - thumb: " . $epObject->thumb);
	...
	$logger->debug("vh1_scrape_prod.php:GetEpisodeData - embed: " . $epObject->url_hi);
	...
	foreach($xml->player->feed->rss->channel->item as $showinfo){
		...
		//$logger->debug("vh1_scrape_prod.php:GetEpisodeData - duration: " . $rt);
		$runtime += $rt;	
	...
	if($entry_exists->result == false){
		...
		echo "..[INSERTING this content for " . $epObject->title . " [Episode]: " . $epObject->episode . " [Season]: " . $epObject->season ." ].. ";
		...
		if($dupcheck == false){
			...
			echo("[\/GetEpisodeData - We already have this episode stream too./\]");	
	}
...
function findImageString($link) {
	
	global $logger;
	global $vh1;
	$logger->debug("vh1_scrape_prod.php:findImageString - Parsing: " . $s);
	$imgstr = "";
	
	$tags = get_meta_tags($link);

	$img_string  = $tags['thumbnail'];
	$img_string   = $vh1.$img_string;
		
	$respos   = explode("/",$img_string);
	$img_name = $respos[count($respos)-1];
		
	$start  = strripos($img_name,"_");
	$finish = strripos($img_name,".jpg");

	if($start > 0){
		$img_name = str_replace(substr($img_name,$start+1,$finish-$start-1),"281x211",$img_name);
		$img_tmp = str_replace($respos[count($respos)-1],$img_name,$img_string);
		//print "new image string $img_tmp <br>";
		
	}else{
		$img_name = "281x211.jpg";
		$img_tmp = str_replace($respos[count($respos)-1],$img_name,$img_string);
		//print "new image string $img_tmp <br>";
	}
		
	if(file_get_contents($img_tmp)&& strlen($img_tmp) > 20){			//incase our conversion to a larger image above doesnt work, we can pull a default thumbnail from metadata
		$imgstr = $img_tmp;
	}elseif(file_get_contents($img_string) && strlen($img_string) > 20){
		$imgstr = $img_string;
	}else{
		$imgstr = "default";
	}
	
	$logger->debug("vh1_scrape_prod.php:findImageString - Result: " . $imgstr);
	return $imgstr;
}
...
function getDescriptions($_url, $_huluurl, $_title, $isHulu) {
	...
	if (!$isHulu) {
		...
		$logger->debug("vh1_scrape_prod.php - Getting Hulu Description! ");
...
function getDescription($_url){
	global $logger;	
	$logger->debug("vh1_scrape_prod.php:getDescription - Getting description with url: " . $_url);
	$scrape = new Scrape();
	$scrape->fetch($_url);

	$data = $scrape->removeNewlines($scrape->result);
	$logger->debug("vh1_scrape_prod.php:getDescription - data: " . $data);
	...
	*/
	...
	$logger->debug("vh1_scrape_prod.php:getDescription - description: " . $desc);
	return $desc;
}
...
function getHuluGenre($data){
	global $logger;	
	$logger->debug("vh1_scrape_prod.php - Getting Hulu genre! ");
		$url = $data;
		$scrape = new Scrape();
		$scrape->fetch($url);
...
function getHuluKeywords($_url){
	...
	global $logger;	
	$logger->debug("vh1_scrape_prod.php:getHuluKeywords() ");
	$scrape = new Scrape();
...
function saveEpisodeRef($e){
	global $t_suffix;
	global $logger;
	$query = "insert into episode_reference". $t_suffix . " (myubi_id,title, description, synopsis, network, url, keywords, genre, keyart, poster, showthumb, userrating, cast, rating, fileloc,type,year) VALUES ('".$e->myubi."','".mysql_escape_string(htmlentities($e->title,ENT_QUOTES, 'UTF-8'))."','".mysql_escape_string(htmlentities($e->description,ENT_QUOTES, 'UTF-8'))."','".mysql_escape_string(htmlentities($e->synopsis,ENT_QUOTES, 'UTF-8'))."','".$e->network."','".$e->url."','".mysql_escape_string(htmlentities($e->keywords))."','".$e->genre."','".$e->keyart."','".$e->poster."','".$e->showthumb."','".$e->userrating."','".$e->actors."','".$e->rating."','".$e->fileloc."','".$e->type."','".$e->year."')";
		 $logger->debug("vh1_scrape_prod.php:saveEpisodeRef -  episode_reference insert query: " . $query);
		if (mysql_query($query)) {
	
			//print "<p>============".$e->title." SHOW REF INSERTED=================<p>";
			$logger->debug("vh1_scrape_prod.php:saveEpisodeRef -  episode_reference INSERTED: ");
			print_r($e);
						
		} else {
			$logger->error("vh1_scrape_prod.php:saveEpisodeRef -  episode_reference insert: " . mysql_error());
...
global $t_suffix;
 ...
 $query = "insert into episode_content". $t_suffix . "(MYUBI_ID,TITLE,EPISODE,SEASON,TYPE,DESCRIPTION,SYNOPSIS,NETWORK,KEYWORDS,COUNTRY,YEAR,URL,SHOWTHUMB,THUMB,POSTER,KEYART,DURATION,GENRE,EPISODETITLE,URLID,PUBDATE,RATING,USERRATING,FILELOC) VALUES ('".$e->myubi."','".mysql_escape_string(htmlentities($e->title,ENT_QUOTES, 'UTF-8'))."','".$e->epnum."','".$e->season."',".$e->type.",'".mysql_escape_string(htmlentities($e->description,ENT_QUOTES, 'UTF-8'))."','".mysql_escape_string(htmlentities($e->synopsis,ENT_QUOTES, 'UTF-8'))."','".$e->network."','".mysql_escape_string(htmlentities($e->keywords,ENT_QUOTES, 'UTF-8'))."','US','".$e->year."','".$e->url."','".$e->showthumb."','".$e->thumb."','".$e->poster."','".$e->keyart."','".$e->duration."','".$e->genre."','".mysql_escape_string(htmlentities($e->episodeTitle,ENT_QUOTES, 'UTF-8'))."','".$e->urlid."','".$e->pubDate."','".$e->rating."','".$e->userrating."','".$e->fileloc."')";
...
function insertEpisodeStream($e){
		...
		if (mysql_query($query)) {
		 print "<p>============EPISODE STREAM" . $e->epnum ." INSERTED=================<p>";
			   //$logger->error("vh1_scrape_prod.php Mysql Query Error episode_streams" . mysql_error());
			
		} else 
			echo "[\**********save episodeStream:InsertError: -> : ".mysql_error()."**********/]";
			//$logger->error("vh1_scrape_prod.php Mysql Query Error episode_streams" . mysql_error());
}
...
function checkEpFiles($_epnum, $_season, $_title){
	global $logger;	
		...
		$res = mysql_query($query);
        	...
        	if($res->error) {
                	$logger->error("vh1_scrape_prod.php Message Mysql SQL Query Error" . mysql_error());
        	}
			...
			if(mysql_num_rows($res) > 0){
				...
				$logger->warn("vh1_scrape_prod.php duplicate found " .  $data['TITLE'] );
				print "<br><i>duplicate found " . $data['TITLE'] . "</i> <br>";
				return (bool) false;
			}else{
				print "<br><i>save it now</i> <br>";
				$logger->info("vh1_scrape_prod.php save it now "  );
				return (bool) true;
			}
}
